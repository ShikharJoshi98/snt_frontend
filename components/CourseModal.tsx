...
import { courseCategories, courses } from "@/constants/Courses";
...
const CourseModal = ({ onClose }: any) => {
    ...
    useEffect(() => {
        if (courseCategories) {
            setSelectedCourse(courseCategories[0]?.courseCategory);
        }
    }, [courseCategories]);

    return (
        <div className="bg-black/50 z-60 fixed inset-0 flex items-center justify-center p-2">
            <div className="bg-[#525fe1] max-h-screen h-[99vh] max-w-[95vw] overflow-y-auto flex flex-col w-full rounded-xl p-4 shadow-lg">
                ...
                <div className="flex md:flex-row flex-col gap-3 md:gap-0 md:justify-center h-[90%]">
                    <div className="md:w-[28%] w-full bg-[#e9ecef] overflow-x-auto p-4 flex flex-col gap-2 rounded-md md:rounded-r-none md:rounded-l-md">
                        ...
                        {courseCategories.map((category, index) => (
                            <div key={index} onClick={() => setSelectedCourse(category?.courseCategory)} className={`hover:bg-gray-400/10 ${selectCourse === category?.courseCategory ? 'bg-gray-400/20' : ''} p-2 rounded-md cursor-pointer`}>
                                <p className="font-semibold text-zinc-700 text-lg">{category?.courseCategory}</p>
                            </div>
                        ))
                        }
                    </div>
                    <div className="bg-white w-full md:w-[72%] flex flex-wrap gap-4 overflow-x-auto p-4 rounded-md md:rounded-l-none md:rounded-r-md">
                        {courses.filter((course, _) => course?.courseCategory === selectCourse).map((course, index) => (
                            <Link href={`/Courses/${course?.id}`} onClick={() => onClose()} key={index} className="p-2.5 shadow-md shadow-gray-300 cursor-pointer group hover:bg-gray-100 duration-300 w-[180px] h-[180px] min-h-48 flex flex-col items-center gap-3 rounded-md">
                                <div className="p-4 rounded-full group-hover:bg-white bg-gray-100">
                                    <Image src={course?.courseLogo} alt={course?.course} height={60} width={60} className="object-cover" />
                                </div>
                                <p className="text-center font-semibold text-zinc-800">{course.course}</p>
                            </Link>
                        ))}
                    </div>
                </div>
